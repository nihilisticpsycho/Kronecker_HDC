...
class HDC(nn.Module):
	def __init__(self, dim, D, num_classes, enc_type='RP', binary=True, device='cuda', kargs=None):
		...
		if enc_type in ['RP', 'RP-COS']:
			...
			#print("RP layer size",self.rp_layer.size())
	...
	def encoding(self, x):
		if self.enc_type == 'RP':
			#print("inside encoding, size of x", x.size())
			out = self.rp_layer(x)
			#print("inside encoding, size of out", out.size())
	...
	def forward(self, x, embedding=False):
		...
		if embedding:
			...
			#print("size of enc_hv", out.size())
			out = self.similarity(class_hvs=binarize_hard(self.class_hvs), enc_hv=out)   
			#print("size of class_hvs", self.class_hvs.size())   
		return out
...
def metric_train(model, loss_func, mining_func, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, labels) in enumerate(train_loader):
        ...
        loss.backward()
        optimizer.step()
        #weight_update = model.binarize_weight(model.rp_layer.weight.clone())
	    
        #if (model.rp_layer):
        #    with torch.no_grad():
        # weight_matrix = model.rp_layer.weight.clone()
        # weight_matrix = np.where(weight_matrix < 0, -1, 1).astype(bool)
        # D = weight_matrix.shape[1]
        # for i in range(D-1):
        #     weight_matrix[:, i+1] = weight_matrix[:, i] ^ weight_matrix[:, i+1]
        # weight_matrix = torch.from_numpy(weight_matrix)		    
        # model.rp_layer.weight.data.copy_(weight_matrix)				
        #if batch_idx % 100 == 0:
           # print(
...
# def train(X_train, y_train, X_test, y_test, D=1024, alg='rp', Kron_shape=[32,28], epoch=20, lr=1.0, L=64, quantize=0):
...
# 	class_norms_fp = [np.linalg.norm(hv) for hv in class_hvs_fp]

# 	class_hvs_best = deepcopy(class_hvs_fp)
# 	class_norms_best = deepcopy(class_norms_fp)
...
# 			for j in pickList:
# 				predict = max_match(class_hvs_fp, train_enc_hvs[j], class_norms_fp)
...
# 				predict = max_match(class_hvs_fp, validation_enc_hvs[j], class_norms_fp)
