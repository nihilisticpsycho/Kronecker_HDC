...
class quantize_layer(nn.Module):
    def __init__(self, size_in, train_x, levels) -> None:
        ...
        weights = torch.tensor(np.percentile(train_x.cpu().flatten().detach(), q)).float()
    ...
    def forward(self, x, soft = False):
        if(soft): #for training
            ...
            for i in range(len(cutoffs) - 1):
                ...
                if(self.levels == 2):
                    res += mask * ((torch.tanh((x-self.weights[i]))) + offsets[i] + 0.5) #rescale for 2 levels
                else:
                    res += mask * ((0.5 * torch.tanh((x-self.weights[i]))) + offsets[i])
...
class HDC(nn.Module):
    def __init__(self,dim, D, num_classes, levels = 3, enc_type='RP', device='cuda', similarity_type="cosine", kargs=None):
    ...
    def init_quantize(self, x_train):
        self.quantize_data = quantize_layer(self.D, self.encoding(x_train, quantize=False), levels= self.levels)
    ...
    def init_class(self, x_train, labels_train):
        with torch.no_grad():
            ...
            self.quantize_class_hvs = quantize_layer(self.D, self.class_hvs, levels= self.levels)
            
            self.class_hv_distribution = self.class_hvs.clone().detach() #for creating plots
            self.class_hvs = nn.parameter.Parameter(self.quantize_class_hvs(self.class_hvs))
    ...
    def HD_train_step(self, x_train, y_train, lr=1):
        with torch.no_grad():
            ...
            for i in range(enc_hvs.size()[0]):
                ...
                self.quantize_class_hvs = quantize_layer(self.D, self.class_hvs_nq, levels= self.levels)
